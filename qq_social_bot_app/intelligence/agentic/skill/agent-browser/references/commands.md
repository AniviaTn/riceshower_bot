...
agent-browser 所有命令的完整参考。快速入门和常见模式请参见 SKILL.md。
...
agent-browser wait --fn "window.ready"     # 等待 JS 条件满足（或 -f）
```
...
```bash
agent-browser eval "document.title"          # 仅限简单表达式
agent-browser eval -b "<base64>"             # 任意 JavaScript（base64 编码）
agent-browser eval --stdin                   # 从 stdin 读取脚本
```

使用 `-b`/`--base64` 或 `--stdin` 以确保可靠执行。嵌套引号和特殊字符的 shell 转义容易出错。

```bash
# 将脚本进行 base64 编码，然后：
agent-browser eval -b "ZG9jdW1lbnQucXVlcnlTZWxlY3RvcignW3NyYyo9Il9uZXh0Il0nKQ=="

# 或使用 stdin 配合 heredoc 处理多行脚本：
...
agent-browser --json ...              # JSON 格式输出便于解析
agent-browser --headed ...            # 显示浏览器窗口（非无头模式）
agent-browser --full ...              # 全页面截图（-f）
...
agent-browser <command> --help        # 显示指定命令的详细帮助
```
...
agent-browser profiler stop trace.json    # 停止并保存分析结果
```
